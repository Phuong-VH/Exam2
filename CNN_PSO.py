...
def third_model(zeropad_1, filt1, filt2, filt3, filtlen_1, filtlen_2, filtlen_3, dropout1, dropout2, dropout3, pol1, pol2, pol3):
    nb_filter1  = filt1;
    nb_filter2  = filt2;
    nb_filter3  = filt3;
    filter1_size = filtlen_1;
    ...
    conv1 = Conv3D(nb_filter1, (filter1_size, filter1_size, filter1_size), activation='relu', padding="same")(zeropad_1)
    max1 = MaxPooling3D(pool_size=(poling1, poling1, poling1), padding="same")(conv1)
    conv2 = Conv3D(nb_filter2, (filter2_size, filter2_size, filter2_size), activation='relu', padding="same")(max1)
    max2 = MaxPooling3D(pool_size=(poling2, poling2, poling2), padding="same")(conv2)
    conv3 = Conv3D(nb_filter3, (filter3_size, filter3_size, filter3_size), activation='relu', padding="same")(max2)
...
def cnn(x):
    ...
    if struct == 1:
        ...
        nn = third_model(zeropad_1, filt1, filt2, filt3, filtlen_1, filtlen_2, filtlen_3, dropout1, dropout2, dropout3, pol1, pol2, pol3)
    elif struct == 4:
    ...
    output = Dense(10, activation='softmax')(hid)
    
    model = Model(inputs=inp_1, outputs=output)

    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    cnn_model = model.fit(X_train, y_train_dense, epochs=100, batch_size=1, verbose=0, validation_data=(X_test, y_test_dense))
    
    score = cnn_model.history['accuracy'][-1]

    t2_0 = datetime.datetime.now()
    
    print('score: ', score, 'values: ', x, 'time: ', t2_0-t1_0)
    return -score 
def log(s):
...
bounds = [(1,5),(8,32),(8,32),(8,32),(2,5),(2,5),(2,5),(0.2,0.5),(0.2,0.5),(0.2,0.5),(2,5),(2,5),(2,5),(32,128),(0.2,0.5)]
NDim = 15;
NParticle =NDim*3
MaxIters=50
